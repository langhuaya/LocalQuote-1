...
export default function App() {
  ...
  const handleProductSave = (product: Product) => {
    const newProducts = products.some(p => p.id === product.id) 
  ...
  const handleCustomerSave = (customer: Customer) => {
    const newCustomers = customers.some(c => c.id === customer.id)
  ...
  const handleExport = async (quote: Quote, format: 'pdf' | 'image') => {
    ...
    setTimeout(async () => {
        if (printRef.current) {
            try {
                // Optimization: Use lower scale for PDF to reduce file size (2 is usually good for A4 print)
                // Use high scale (3) for Image export so it looks crisp on high-res phones
                const scale = format === 'pdf' ? 2 : 3;

                // 1. Capture the full height component
                ...
                if (format === 'image') {
                    // --- IMAGE EXPORT ---
                    // PNG is lossless, good for sharing single images
                    ...
                    // --- PDF EXPORT ---
                    // Optimization: Use JPEG with 0.75 quality to drastically reduce file size
                    // PNG creates massive PDFs (30MB+), JPEG reduces this to ~1-2MB
