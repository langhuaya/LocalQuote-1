...
export const BrandsManager: React.FC<BrandsManagerProps> = ({ brands, onSave, onDelete, t }) => {
  ...
  // --- Supplier Logic (Duplicated from ProductsManager for independence) ---
    ...
    <div className="space-y-6">
      {isEditing ? (
        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
          <h3 className="font-bold text-xl text-gray-800 mb-6">{currentBrand.id ? 'Edit Brand' : 'Add Brand'}</h3>
              ...
              <div className="pt-4 border-t">
                 <div className="flex justify-between items-center mb-4">
                    <h4 className="text-sm font-bold text-orange-600 uppercase flex items-center">
                        <Truck size={16} className="mr-2" /> Suppliers for this Brand
                    </h4>
                 ...
                 <div className="space-y-3">
                    {editingSuppliers.length === 0 && <p className="text-sm text-gray-400">No specific suppliers listed.</p>}
                    {editingSuppliers.map((supplier, index) => (
                        <div key={supplier.id} className="p-4 rounded-lg border border-gray-200 bg-gray-50">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <input 
                                    className="w-full p-2 border rounded text-sm"
                                    placeholder="Contact / Link / Notes"
                    ...
                    <div key={brand.id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow relative group">
                        <div className="flex justify-between items-start">
                             <div>
                                ...
                                <p className="text-xs text-gray-500 mt-1">{brand.suppliers?.length || 0} Suppliers</p>
                             </div>
