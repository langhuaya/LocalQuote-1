...
export const InvoiceTemplate = forwardRef<HTMLDivElement, InvoiceTemplateProps>(({ quote, settings, mode = 'preview', showImages = true }, ref) => {
  ...
  // A4 Layout Constants
  const baseWidth = 794; // 210mm @ 96dpi
  ...
  if (mode === 'preview') {
    ...
    wrapperClass = "absolute top-0 left-[-9999px]"; 
  } else {
    ...
    <div className={wrapperClass}>
       <div ref={ref} style={containerStyle} className="flex flex-col">
          ...
          <div className="p-10 flex flex-col flex-grow relative">
            ...
            {/* --- HEADER --- */}
            <div className="flex justify-between items-start mb-8 no-split">
                ...
                <div className="w-1/2 text-right">
                    ...
                    <div className="inline-block text-left min-w-[200px]">
                        <div className="grid grid-cols-[80px_1fr] gap-y-1 text-sm border-l-4 border-blue-600 pl-4 py-1">
                    ...
                    <tbody className="text-sm">
                        {items.map((item, idx) => (
                            <tr key={item.id} className="border-b border-slate-100 last:border-0 no-split">
                                ...
                                <td className="py-3 align-top text-center text-slate-600">{item.brand || '—'}</td>
                                <td className="py-3 align-top text-left text-slate-600 text-xs">{item.leadTime || '-'}</td>
                                <td className="py-3 align-top text-right font-medium">
                                    {item.quantity} <span className="text-xs text-slate-400 font-normal">{item.unit}</span>
                                </td>
                                <td className="py-3 align-top text-right text-slate-600 pr-8">{fmt(item.price)}</td>
                                <td className="py-3 align-top text-right font-bold text-slate-800">{fmt(item.price * item.quantity)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            {/* --- TOTALS & BANK INFO --- */}
            <div className="flex justify-between items-start mb-8 no-split pt-4 border-t border-slate-200">
                <div className="w-[58%] pr-8">
                     <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider mb-2">Payment Details</h4>
                     <div className="bg-slate-50 p-4 rounded border border-slate-100 text-[11px] text-slate-600 font-mono leading-normal whitespace-pre-wrap">
                        {settings.bankInfo}
                     </div>
            ...
            <div className="mt-auto no-split border-t border-slate-100 pt-8">
                         ...
                         <div className="h-28 w-full relative flex flex-col items-center justify-end pb-2">
                             {settings.stampDataUrl && (
                                 <img 
                                    ...
                                    alt="Stamp"
                                    className="absolute bottom-4 left-1/2 -translate-x-1/2 w-36 h-36 object-contain mix-blend-multiply opacity-90 pointer-events-none"
                ...
                <div className="text-center mt-10 text-[10px] text-slate-400 uppercase tracking-widest">
